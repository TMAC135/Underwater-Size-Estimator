        ...
        G=230; %the parameter to determine the depth of object if the distance is less than 0.5m, choose G=250, if the distance is in [0.5m,1m]; choose G=215;if the distance is greater than 1m, choose G=150;

        %read lower camera 
        readerobj = VideoReader('lower137.avi', 'tag', 'myreader1');   % Construct a multimedia reader object associated with file 'xylophone.mpg' with
        ...
        %read upper camera
        readerobj = VideoReader('upper137.avi', 'tag', 'myreader1');   % Construct a multimedia reader object associated with file 'xylophone.mpg' with
               % user tag set to 'myreader1'
         vidFrames = read(readerobj);% Read in all video frames.
         numFrames = get(readerobj, 'NumberOfFrames');% Get the number of frames.
            for i=1:frameinterval:numFrames
            ...
            BW1 = bwareaopen(BW1,50);%subplot(2,1,2);imshow(BW1,[]);title('Soble Edge Detection with Gaussian Filter for lower image')

           %upper image after motion detection and edge detection
            im5=imread('upper25.jpg');%subplot(2,1,1);imshow(im1);title('upper image at first frame');
            im6=imread('upper73.jpg');%subplot(2,1,2);imshow(im2);title('upper image at second frame');
            ...
            BW2 = bwareaopen(BW2,50);%subplot(2,1,2);imshow(BW2,[]);title('Soble Edge Detection with Gaussian Filter for upper image')
            ...
            d=abs(stats2(m2).BoundingBox(1,2)-stats1(m1).BoundingBox(1,2));%parallax coefficient in pixcels
            D=b*(G)/(1*d); 
            L=b*.5*(stats1(m1).BoundingBox(1,3)+stats2(m2).BoundingBox(1,3))/(1*d);
            ...
            figure;imshow(im1);
             ...
             if D>20
              ...
              rectangle('Position',bb2,'EdgeColor','r','LineWidth',2)
                a=text(bc(1)-120,bc(2)-35, strcat('Depth(m): ', num2str(D), '    Length(m): ', num2str(L)));
